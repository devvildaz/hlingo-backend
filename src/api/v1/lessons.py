...
from src.api.v1.contracts.lesson import GradeDTO, LessonPost
from src.core.schemas.Lessons import Grade, Lesson
...
def get_lesson_by_id(lesson_id: str):
    try:
        ...
        return {"message": f"{lesson_id} is not a valid id"}

    if not lesson_found:
        return {"message": f"Lesson with id {lesson_id} not found"}
...
async def post_lessons(lesson: LessonPost):
    new_lesson = Lesson(
        ...
        example_video=lesson.example_video,
...
async def get_grade_by_user_id_lesson_id(user_id: str, lesson_id: str):
    try:
        ...
        return {"message": f"{user_id} or {lesson_id} is not a valid id"}
    if not grade_found:
        return {"message": f"{user_id} or {lesson_id} not found"}
